...
export default function AdminDashboard() {
  ...
  const getInsName = useCallback(
    id => (instructors.find(i => i.id.toString() === id.toString()) || {}).username || '',
    [instructors]
  );
  ...
  // kick off fetchData(), but effect returns nothing
  useEffect(() => {
    fetchData();
  }, []);
  ...
  const filteredSystem = systemActivity.filter(e =>
  ...
  return (
    <div className={`${styles.container} ${dark?styles.dark:''}`}>
      ...
      <section className={styles.statsGrid}>
        <div className={styles.card}><FaUsers className={styles.iconUser}/>
          <div><h2>{stats.totalUsers}</h2><p>Users</p></div>
        </div>
        <div className={styles.card}><FaBook className={styles.iconCourse}/>
          <div><h2>{stats.totalCourses}</h2><p>Courses</p></div>
        </div>
        <div className={styles.card}><FaClipboardList className={styles.iconEnroll}/>
          <div><h2>{stats.totalEnrollments}</h2><p>Enrollments</p></div>
        </div>
        <div className={styles.card}><FaChalkboardTeacher className={styles.iconInstructor}/>
          <div><h2>{stats.totalInstructors}</h2><p>Instructors</p></div>
        </div>
        <div className={styles.card}><FaUserGraduate className={styles.iconStudent}/>
